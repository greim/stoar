...
Each data item consists of a definition, which is an object with `type`, `value` and `validate` properties.
  ...
  * **loadable** - Indicates that this property is associated with a remote data source. Setting this to `true` merely causes some extra properties to be added. For example if the property is called `posts`, then properties called `posts:loading`, `posts:status`, `posts:code`, and `posts:timestamp` will be created too, each with a corresponding `type`. It's up to you whether and how to use these extra properties.
